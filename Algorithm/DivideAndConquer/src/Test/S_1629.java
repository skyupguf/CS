...
*   풀이과정
*   입력되는 모든 수가 최대 maximum 값으로 단순 제곱 계산으로는 메모리 오버플로우나 20억번 연산이 발생한다.
*   이를 해결하기 위해 수학적 지식이 필요하다.
*   1. 지수 법칙 : A^(B) = A^(a + b) = A^a * A^b
*   2. 모듈러 성질 : (A * B) % C = ((A % C) * (B % C)) % C

*   1. 지수분할 과정을 분할하면서 적용하면
    10^11 = (10^5)                                 * (10^5)                                 * (10^1)
            (10^2 * 10^2 * 10^1)                   * (10^2 * 10^2 * 10^1)                   * (10^1)
            ((10^1 * 10^2) * (10^1 * 10^2) * 10^1) * ((10^1 * 10^2) * (10^1 * 10^2) * 10^1) * (10^1)

*   2. 지수를 계속 곱해서 리턴하면 마찬가지로 최대수가 맥시멈이므로 오버플로우가 나올 수 있다.
*   따라서, 모듈러 성질을 적용해서 매 곱에서 나머지 연산을 수행한다.
    홀수면 10^5일 경우 (10^2 * 10^2 % m) * 10^1 % m
    짝수면 10^2 * 10^2 % m
* */
...
public class S_1629 {
    ...
    public static long calPowAndMod (long i, long e, long m) {
        // 지수를 분할해 최소단위인 1일 경우 그대로 정수 i를 %m 이후 리턴한다. 10^1 의 경우
        if (e == 1) {
            return i % m;
        }

        // 지수가 짝수일 경우나 홀수일 경우 둘 다 지수를 절반으로 분할하는 건 마찬가지이므로 2로 나눠 재귀호출 한다.
        // 둘 로 나눠 재귀호출할 필요가 없는 이유는 두 경우로 나눠지는 경우가 완전히 동일하기 때문이다.
        // 예를 들어, 10^4 = 10^2 * 10^2 로 한쪽만 구하고 제곱을 하면 그만이다.
