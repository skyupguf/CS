...
/*
//  접근방법  //
    노드를 구현하는 class는 트리의 다른 노드들과 상관없이 자체적으로 기능할 수 있어야 하므로 값과 다른 자식노드를 추가할 변수들이 필요하다.
    트리를 구현하는 class는 트리의 루트가 위치할 변수와 루트를 기준으로 자식노드를 추가하고 확인하는 메서드가 필요하다.
    
    노드를 추가할 때 root가 존재하지 않으면 root에 시작점이 되는 노드를 추가한다.
    자식노드를 추가할 때 노드가 이미 존재하는지 확인할 수 있어야 하며, 특정 노드의 자식노드로 추가하기 위해 부모노드의 값도 확인할 수 있어야 한다.
    부모노드의 위치를 찾기 위해서 모든 계층의 노드를 탐색할 수 있는 
