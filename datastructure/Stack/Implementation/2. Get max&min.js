...
/*
//  접근방법  //
    스택에 데이터가 추가될 때와 추출될 때 최대, 최소값이 어떻게 갱신되는지 경우를 나열해 본다.
    1. 스택이 비어있을 경우 최대, 최소값은 동일하다.
    2. 스택에 데이터가 삽입될 경우 최소, 최대값과 비교하여 갱신할 수 있어야 한다.
    3. 스택에 데이터가 추출될 경우 최소, 최대값을 확인하고 이전 최소, 최대값으로 갱신할 수 있어야 한다.
...
//  복잡도  //
    각 메서드는 모두 스택로직을 기반으로 하기 때문에 O(1)의 시간복잡도로 모두 작동된다.
...
/*
//  접근방법  //
    추가공간을 사용하지 않으려면 이전값을 기억하는 유일한 공간은 stack이 유일하다.
    따라서, 스택에 추가하려는 데이터를 최대, 최소값과의 수식의 결과로 저장해 이전 값을 기억하도록 한다.
    또한, 이 변형된 입력값은 추출하거나 확인할 때 원본값으로 리턴되도록 해야한다.
    ...
    2. 최대값 < 추출값인 경우 (추출값 - 최대값)을 최대값으로 갱신하고 스택에서 추출값을 제거, 최대값을 리턴한다.
...
//  수도코드  //
    ...
    2. push메서드에서 stack의 데이터 존재여부, 최대, 최소값의 갱신여부로 각각 분기한다.
        ...
        2-2. 스택에 데이터가 존재할 경우에 다시 3가지의 경우로 분기한다.
            a. min > data의 경우 (data - min)을 stack에 삽입하고 min을 data로 갱신한다.
            b. max < data의 경우 (data + max)을 stack에 삽입하고 max를 data로 갱신한다.
            c. 둘다 해당되지 않으면 stack에 그냥 삽입한다.
        2-3. 모든 작업이 완료됐으면 top을 1누적한다.
    
    3. pop메서드는 추출되는 3가지 경우를 고려한다.
        3-1. 임시변수 temp를 선언하고 추출할 스택값을 할당한다.
        3-2. min > temp일 경우 (min - temp)로 이전 최소값을 복원하여 현재 min에 할당하고 temp엔 복원전 min을 할당한다.
        3-3. max < temp일 경우 (temp - max)로 이전 최대값을 복원하여 현재 max에 할당하고 temp엔 복원전 max를 할당한다.
